...
async function verifyGallery(page, viewport) {
  ...
  const viewerChrome = await page.evaluate(() => {
    const dialog = document.querySelector('#product-media-overlay');
    const isVisible = element => Boolean(element && element.getClientRects().length && getComputedStyle(element).display !== 'none');
    ...
    const productLink = [...dialog.querySelectorAll('.cp-media-product-link')].find(isVisible);
    const motion = dialog.querySelector('.cp-media-jump');
    const counter = dialog.querySelector('.cp-media-header-status .cp-eyebrow');
    const close = dialog.querySelector('.cp-media-icon-button');
    const panel = dialog.querySelector('.cp-media-panel');
    const header = dialog.querySelector('.cp-media-dialog-header');
    const rects = {
      motion: rectFor(motion),
      product: rectFor(productLink),
      counter: rectFor(counter),
      close: rectFor(close),
    };
    const chromeRects = Object.values(rects).filter(Boolean);
    const collision = chromeRects.some((rect, index) => chromeRects.slice(index + 1).some(other => overlaps(rect, other)));
    const panelRect = panel.getBoundingClientRect();
    const withinPanel = chromeRects.every(rect => rect.x >= panelRect.x - 1
      && rect.right <= panelRect.right + 1
      && rect.y >= panelRect.y - 1
    ...
    return {
      ...
      productPlacement: productLink?.closest('.cp-media-index') ? 'index' : 'header',
      productSingleLine: productLink ? productLink.scrollWidth <= productLink.clientWidth + 1 && productLink.getBoundingClientRect().height <= 45 : false,
      visibleProductLinkCount: [...dialog.querySelectorAll('.cp-media-product-link')].filter(isVisible).length,
    };
  });
...
async function verifyRegressionRoute(page, viewport, route) {
  ...
  const state = await page.evaluate(async () => {
    ...
    const visibleAndAccessibleCopy = [
      ...
      ...[...document.querySelectorAll('figcaption')].map(element => element.textContent),
    ].join(' ');
    const forms = [...document.forms];
    const enabledSubmitControls = forms.flatMap(form => [...form.querySelectorAll('button[type="submit"]:not(:disabled), input[type="submit"]:not(:disabled)')]);
    const enabledSelects = forms.flatMap(form => [...form.querySelectorAll('select:not(:disabled)')]);
    const checkoutLinks = [...document.querySelectorAll('a[href]')].filter(link => /checkout/i.test(`${link.textContent} ${link.getAttribute('href')}`));
    return {
      ...
      brokenVideos: videos.filter(video => video.error).length,
      providerLeak: /\b(?:shopify|modelize|apliiq|generated by)\b/i.test(visibleAndAccessibleCopy),
      formCount: forms.length,
      enabledSubmitCount: enabledSubmitControls.length,
      enabledSelectCount: enabledSelects.length,
      checkoutLinkCount: checkoutLinks.length,
      continueToCheckoutCount: (document.body.innerText.match(/continue to checkout/gi) || []).length,
      selectionDisabledCount: (document.body.innerText.match(/selection disabled/gi) || []).length,
      purchasingDisabledCount: (document.body.innerText.match(/purchasing disabled/gi) || []).length,
      mainBackground: getComputedStyle(document.querySelector('main')).backgroundColor,
    };
  });
  if (route.name !== 'home-regression') {
    await page.screenshot({ path: `${screenshotRoot}/${viewport.name}-${route.name}.png` });
  }
  return { name: route.name, path: route.path, status: response?.status() || 0, ...state };
}

async function verifyRegressionRoutes(page, viewport) {
  const routes = [
    { name: 'home-regression', path: '/' },
    { name: 'shop', path: '/shop' },
    { name: 'pdp', path: '/products/carlophillips-signature-hoodie' },
    { name: 'bag', path: '/bag' },
  ];
  const results = [];
  for (const route of routes) results.push(await verifyRegressionRoute(page, viewport, route));
  return results;
}
...
try {
  for (const viewport of viewports) {
    ...
    console.log(`qa:${viewport.name}:routes`);
    const routes = await verifyRegressionRoutes(page, viewport);
...
for (const viewport of ['desktop', 'compact', 'mobile']) {
  for (const state of ['home-hero', 'home-menu', 'home-one', 'home-overlay', 'shop', 'pdp', 'bag']) {
...
report.summary = {
  ...
  screenshots: report.viewports.length * 9,
  ...
  galleryPass: report.viewports.every(item => item.gallery.bottomRailCount === 0
    ...
    && item.gallery.viewerChrome.visibleProductLinkCount === 1
    && item.gallery.viewerChrome.withinPanel
    && !item.gallery.viewerChrome.collision
    && !item.gallery.viewerChrome.headerOverflow
    && item.gallery.viewerChrome.productSingleLine
    && item.gallery.viewerChrome.productPlacement === (item.width < 640 ? 'index' : 'header')
  ...
  regressionRoutesPass: report.viewports.every(item => item.routes.every(route => route.status === 200
    && route.meaningfulContent
    && !route.frameworkOverlay
    && !route.horizontalOverflow
    && route.brokenImages === 0
    && route.brokenVideos === 0
    && !route.providerLeak)),
  checkoutFailClosedPass: report.viewports.every(item => item.postRequests.length === 0
    && item.routes.every(route => route.formCount === 0
      && route.enabledSubmitCount === 0
      && route.enabledSelectCount === 0
      && route.checkoutLinkCount === 0
      && route.continueToCheckoutCount === 0)
    && item.routes.find(route => route.name === 'pdp')?.purchasingDisabledCount > 0),
};
